...
const RESOURCE_SLUGS=['find-creative-talent','creative-portfolio-guide','creative-production-network','creative-career-networking']
...
export default async function handler(req,res){
  try{
    ...
    const urls=[{loc:`${BASE}/`,priority:'1.0',changefreq:'weekly'},{loc:`${BASE}/creatives`,priority:'0.9',changefreq:'weekly'},{loc:`${BASE}/resources`,priority:'0.8',changefreq:'weekly'},...RESOURCE_SLUGS.map(slug=>({loc:`${BASE}/resources/${slug}`,priority:'0.7',changefreq:'monthly'}))]
    ...
    const xml=`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${unique.map(u=>`<url><loc>${esc(u.loc)}</loc>${u.lastmod?`<lastmod>${esc(u.lastmod)}</lastmod>`:''}<changefreq>${u.changefreq}</changefreq><priority>${u.priority}</priority></url>`).join('')}</urlset>`
