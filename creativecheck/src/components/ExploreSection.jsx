...
function CommunitySignupPrompt({onClose}){return <div role="dialog" aria-modal="true" onClick={onClose} style={{position:'fixed',inset:0,zIndex:120,background:'rgba(17,19,24,.68)',display:'grid',placeItems:'center',padding:22}}><div onClick={e=>e.stopPropagation()} style={{width:'min(520px,100%)',background:'#f4f1eb',padding:'34px'}}><button type="button" onClick={onClose} aria-label="Close" style={{float:'right',border:0,background:'transparent',fontSize:26,cursor:'pointer'}}>×</button><div style={{fontSize:10,letterSpacing:'.16em',fontWeight:800,color:'#3158c7'}}>CREATIVECHECK · COMMUNITY</div><h2 style={{fontFamily:'Georgia,serif',fontSize:42,fontWeight:400,margin:'12px 0'}}>Want to see more?</h2><p style={{lineHeight:1.7,color:'#625e57'}}>If you want to see more information about the CreativeCheck community, please create your account.</p><a href="#membership" onClick={onClose} style={{display:'inline-block',marginTop:12,padding:'13px 18px',background:'#203b88',color:'#fff',fontSize:10,textTransform:'uppercase',textDecoration:'none'}}>Create Your Account →</a></div></div>}
...
export default function ExploreSection({searchQuery=''}){
  ...
  return <section id="members" style={{maxWidth:1280,margin:'0 auto',padding:'0 5.5vw 45px'}}>
    ...
    {selectedProfile==='__signup__'&&<CommunitySignupPrompt onClose={()=>setSelectedProfile(null)}/>} {selectedProfile&&selectedProfile!=='__signup__'&&member&&<ProfileModal profile={selectedProfile} onClose={()=>setSelectedProfile(null)}/>}</section>
}
...
function ProfileModal({profile,onClose}){const displayName=publicName(profile);const website=safeExternalUrl(profile.website);const instagram=safeExternalUrl(profile.instagram);const portfolio=safeExternalUrl(profile.portfolio_url);useEffect(()=>{if(profile?.id)track('profile_viewed',{profile_id:profile.id})},[profile?.id]);return <div style={{position:'fixed',inset:0,zIndex:100,background:'rgba(17,19,24,.72)',backdropFilter:'blur(10px)',display:'grid',placeItems:'center',padding:24}} onClick={onClose}><div onClick={e=>e.stopPropagation()} style={{width:'min(700px,100%)',maxHeight:'88vh',overflow:'auto',background:'#f4f1eb',padding:40,border:'1px solid rgba(255,255,255,.2)'}}><div style={{display:'flex',justifyContent:'space-between',gap:20,alignItems:'start'}}><div><div style={{fontFamily:'Georgia,serif',fontSize:'clamp(34px,5vw,54px)',lineHeight:.95}}>{displayName}</div><div style={{marginTop:9,color:'#203b88',fontWeight:800,fontSize:14}}>{profile.profession||'Creative Professional'}</div><div style={{marginTop:6,color:'#77736b',fontSize:13}}>{[profile.city,profile.country].filter(Boolean).join(', ')}</div></div><button onClick={onClose} style={{fontSize:28,cursor:'pointer'}}>×</button></div>{profile.bio&&<p style={{fontSize:15,lineHeight:1.8,color:'#625e57',marginTop:28}}>{profile.bio}</p>}<div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:30}}>{website&&<a href={website} target='_blank' rel='noopener noreferrer' style={{padding:'13px 17px',background:'#203b88',color:'#fff',fontSize:10}} onClick={()=>track('outbound_link_clicked',{profile_id:profile.id,href:website})}>Website ↗</a>}{instagram&&<a href={instagram} target='_blank' rel='noopener noreferrer' style={{padding:'13px 17px',border:'1px solid rgba(17,19,24,.2)',fontSize:10}} onClick={()=>track('outbound_link_clicked',{profile_id:profile.id,href:instagram})}>Instagram ↗</a>}{portfolio&&<a href={portfolio} target='_blank' rel='noopener noreferrer' style={{padding:'13px 17px',border:'1px solid rgba(17,19,24,.2)',fontSize:10}} onClick={()=>track('outbound_link_clicked',{profile_id:profile.id,href:portfolio})}>External work ↗</a>}</div></div></div>}
