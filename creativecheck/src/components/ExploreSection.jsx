...
function ProfileCard({ profile, onOpen }) {
  ...
  return <article className="member-compact-card" onClick={() => onOpen(profile)} role="button" tabIndex={0} onKeyDown={e => { if(e.key==='Enter' || e.key===' ') onOpen(profile) }}>
    <div className="member-card-top"><span className="member-initials">{initials}</span>{profile.verified && <span className="member-verified">Verified</span>}</div>
...
export default function ExploreSection({ searchQuery = '' }) {
  ...
  async function fetchProfiles() { try { const response = await fetch(`${SUPABASE_URL}/rest/v1/profiles?status=eq.approved&select=*&order=created_at.desc`, { headers:{ apikey:SUPABASE_KEY, Authorization:`Bearer ${SUPABASE_KEY}` } }); const data = await response.json(); setProfiles(Array.isArray(data) ? data : []) } catch (error) { console.error('Profile fetch error:', error); setProfiles([]) } finally { setLoading(false) } }
  ...
  return <section style={{maxWidth:1280,margin:'0 auto',padding:'0 5.5vw 45px'}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'end',gap:20,borderBottom:'1px solid rgba(17,19,24,.13)',paddingBottom:16,marginBottom:18}}><div><div style={{fontSize:10,letterSpacing:'.22em',fontWeight:800,color:'#77736b'}}>03 / OUR CREATIVE COMMUNITY</div><h2 style={{fontFamily:'Georgia,serif',fontSize:'clamp(42px,5vw,68px)',lineHeight:.9,fontWeight:400,letterSpacing:'-.05em',margin:'10px 0 0'}}>Meet the<br/><em style={{color:'#5d6670'}}>community.</em></h2></div><div style={{display:'flex',gap:4,border:'1px solid rgba(17,19,24,.14)',padding:3}}>{[['all','All'],['creatives','Creatives'],['businesses','Business']].map(([key,label])=><button key={key} onClick={()=>setFilter(key)} style={{padding:'9px 12px',fontSize:10,textTransform:'uppercase',letterSpacing:'.1em',background:filter===key?'#171717':'transparent',color:filter===key?'#fbfaf7':'#66625b',cursor:'pointer'}}>{label}</button>)}</div></div>
    ...
    {!loading && visible.length>0 && <>{filter==='all' ? <>{creatives.length>0 && <MemberGroup title="Creatives" count={creatives.length} profiles={creatives} onOpen={setSelectedProfile}/>} {businesses.length>0 && <MemberGroup title="Business" count={businesses.length} profiles={businesses} onOpen={setSelectedProfile}/>}</> : <MemberGroup title={filter==='businesses'?'Business':'Creatives'} count={visible.length} profiles={visible} onOpen={setSelectedProfile}/>}</>}
