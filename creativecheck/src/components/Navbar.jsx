...
export default function Navbar() {
  ...
  return (
    <nav className={`sticky top-0 z-50 backdrop-blur-md border-b ${dark ? 'bg-[#0b1020]/90 border-white/10' : 'bg-[rgba(253,248,248,0.9)] border-black/[0.07]'}`}>
      <div className="max-w-[1300px] mx-auto px-8 h-[68px] flex items-center justify-between gap-6">
        ...
        <a href="#discover" className="flex items-center gap-2.5 flex-shrink-0">
          ...
          <span className="text-[19.5px] font-extrabold tracking-tight leading-none">
            <span className={dark ? 'text-white' : 'text-gray-900'}>Creative</span>
        ...
        <div className="flex items-center gap-10 flex-1 justify-center">
          <div className="relative">
            <button onClick={() => setOpenMenu(openMenu === 'categories' ? null : 'categories')} className={`text-[15px] font-medium ${dark ? 'text-gray-200' : 'text-gray-600'}`}>
              Categories
            </button>
          ...
          <a href="#resources" className={`text-[15px] font-medium ${dark ? 'text-gray-200' : 'text-gray-600'}`}>
            Resources
          </a>

          <a href="#about" className={`text-[15px] font-medium ${dark ? 'text-gray-200' : 'text-gray-600'}`}>
            About
          </a>
        </div>
        ...
        <div className="flex items-center gap-2.5 flex-shrink-0">
          ...
          <button
            ...
            className={`w-10 h-10 rounded-full border flex items-center justify-center transition-colors ${
              dark ? 'bg-white text-gray-900 border-white' : 'bg-white text-gray-500 border-gray-200'
            }`}
          >
